...
local C = {
    ...
    bg         = Color3.fromRGB(8,   8,   22),   -- fundo principal
    header     = Color3.fromRGB(14,  14,  34),   -- barra de título
    card       = Color3.fromRGB(20,  20,  44),   -- cards internos
    panel      = Color3.fromRGB(26,  26,  52),   -- painéis secundários
    inputBg    = Color3.fromRGB(16,  16,  38),   -- fundo de inputs

    -- Acentos (gradiente azul → roxo → rosa)
    accent     = Color3.fromRGB(99,  102, 241),  -- azul-violeta principal
    accentGlow = Color3.fromRGB(139, 92,  246),  -- roxo brilhante
    accentPink = Color3.fromRGB(236, 72,  153),  -- rosa vibrante
    accentCyan = Color3.fromRGB(56,  189, 248),  -- ciano/azul claro

    -- Estados
    green      = Color3.fromRGB(34,  197, 94),  -- sucesso
    red        = Color3.fromRGB(239, 68,  68),   -- perigo
    redDark    = Color3.fromRGB(153, 27,  27),   -- vermelho escuro

    -- Textos
    text       = Color3.fromRGB(240, 240, 255),  -- texto principal
    subtext    = Color3.fromRGB(140, 140, 200),  -- texto secundário

    -- Bordas e divisores
    divider    = Color3.fromRGB(45,  45,  85),   -- divisor sutil
    border     = Color3.fromRGB(55,  55,  100),  -- borda de cards

    -- Botões
    btnBlue    = Color3.fromRGB(45,  55,  130),  -- botão primário
    btnDark    = Color3.fromRGB(30,  30,  65),   -- botão secundário
    toggleOn   = Color3.fromRGB(139, 92,  246), -- toggle ativo (roxo)
    toggleOff  = Color3.fromRGB(45,  45,  80),   -- toggle inativo
    hold       = Color3.fromRGB(236, 72,  153),  -- modo hold (rosa)
}
...
-- ═══════════════════════════════════════════
--         PAINEL PRINCIPAL (redesenhado Nexus)
-- ═══════════════════════════════════════════
local PW, PH = 540, 360
...
-- Background gradient sutil (azul → roxo)
...
task.spawn(function()
    ...
    while screenGui.Parent do
        t = (t + 0.004) % 1
        -- Hue cycle: azul(0.65) → roxo(0.75) → rosa(0.85) → azul
...
local function createCPSSelector(yPos, parent)
    ...
    local defBtn = makeBtn("Padrão", 52, ROW_Y, 68, ROW_H, f, C.btnBlue)
    defBtn.TextSize = 11
...
print("✅ Phantom GUI v6.0 carregada — Design Nexus!")
print("   • Paleta: Azul · Roxo · Rosa")
